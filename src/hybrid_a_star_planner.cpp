...
namespace HybridAStar
{
    ...
    bool HybridAStarPlanner::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal, std::vector<geometry_msgs::PoseStamped> &plan)
    {
        ...
        Clear(plan);
        DLOG(INFO) << "Got a start: " << start.pose.position.x << " " << start.pose.position.y << " " << Utility::ConvertRadToDeg(start.pose.position.z) << " and a goal: " << goal.pose.position.x << " " << goal.pose.position.y << " " << Utility::ConvertRadToDeg(goal.pose.position.z);
